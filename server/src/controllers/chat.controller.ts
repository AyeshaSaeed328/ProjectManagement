...
const getOrCreateOneOnOneChat = asyncHandler(
  ...
  createdChat.participants.forEach(participant => {
    ...
    return res
      .status(201)
      .json(
        new ApiResponse<ChatWithDetails>(200, createdChat, "Chat created successfully")
      );
  }
)
...
const getUserChats = asyncHandler(
  async (req: Request, res: Response): Promise<Response<ApiResponse<ChatWithDetails[]>>> => {
    ...
    return res.status(200).json(new ApiResponse<ChatWithDetails[]>(200, chats || [], "Chats retrieved successfully"));
  }
);
